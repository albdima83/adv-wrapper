...
export async function getConsentData(
  ...
  try {
    ...
    const hasPurposeConsent = requiredPurposeId
      ? !!tcModel.purposeConsents?.has(requiredPurposeId)
      : true;

    const hasVendorConsent = requiredVendorId
      ? !!tcModel.vendorConsents?.has(requiredVendorId)
      : true;
